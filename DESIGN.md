...
**Implementation note**: the system described below was authored against the original vanilla-TypeScript `client/` and later ported 1:1 (same tokens, same components, same motion) into an Angular rewrite at `client-angular/`, which is now the only shipping client. File-path references throughout this document point at whichever implementation was current when that phase was written; treat them as historical unless a phase explicitly says otherwise.
...
- **`.line-rule` / `.station-tick` / `.board-badge` / `.platform-panel`** (ported into `client-angular/src/styles/board-badge.scss` and the individual feature components that use `.line-rule`/`.station-tick`) — the shared primitives every screen builds on: a colored rail segment, a stop marker, a departure-board-style numeric tag, and a nested panel surface.
...
- **Identidad, no saludo**: `.user-info` in the menu header is now a clickable `.ww-id-chip` — the avatar plus the username in the serif display face — opening the new `#profile-screen`, not a sentence of copy. Sized up to `--md` (64px) and paired with the tier badge (Fase 9's `eloTier()`, same as the profile page) after direct user feedback that the menu felt bland and the avatar — now a real illustrated character, not a color+icon roundel — was nearly invisible at chip size. `.user-info` itself got a touch more padding, and the tier badge's background swapped to `--ww-surface-2` inside the chip since its default `--ww-bg-2` would otherwise match the bar it sits on and disappear.
...
- **Tier = real ELO, not invented XP**: the "carnet" reads a rank straight from the existing ELO field via a client-side `eloTier()` lookup — no new stat invented for the sake of a progress bar. The tier colors are the same line colors already carrying meaning elsewhere (mode pairing, avatar), extended here to mean "which line you ride" competitively — the first five tiers each take one, and the top tier (Mítico) uses the porcelain ink rather than a sixth hue, so it reads as "past the map" instead of one more line; deliberately kept as a separate small pill+progress element rather than reusing the avatar's own circular shape, so the two don't visually compete over what a color means in that spot.
- **ELO baseline**: players start at 0 ELO (not the earlier 1200) with K=60, tuned so a win against an equal-rated opponent is worth ~30 ELO (`round(60 × (1 − expectedScore))`; more for beating a higher-rated opponent, less for beating a lower-rated one — standard ELO). `User.updateElo()` clamps the result at 0 so a loss can't push a new player negative; the schema's `min: 0` validator is the backstop. Tiers are 500 ELO wide — about 17 net wins to climb one, ~50 to reach Maestro and ~83 to reach Mítico:

  | Tier | ELO range | Color |
  |---|---|---|
  | Aprendiz | 0 – 499 (starting tier) | cobalt |
  | Aficionado | 500 – 999 | lime |
  | Experto | 1000 – 1499 | amber |
  | Maestro | 1500 – 1999 | scarlet |
  | Leyenda | 2000 – 2499 | violet |
  | Mítico | 2500+ | porcelain (`--ww-text`) |

- **Rank emblems**: each tier also has an arcade pixel-art emblem (`client-angular/public/assets/ranks/<tier>.png`, 256px, transparent) that escalates in ornament with the rank — plain medal, laurel medal, star shield, winged shield with quills, crowned violet shield, pearl medallion with wings. Generated with GPT Image 2 from a shared prompt template (flat navy background, no lettering), the background keyed out locally and every emblem scaled by one common factor so a higher tier reads as physically larger. They are detailed illustrations, so `ww-rank-emblem` is used large only — the VS screen (pinned to the outer shoulder of each avatar), the profile header, and the results ELO panel (where it swaps at the exact ELO that crosses a tier boundary and a "¡Subes a …!" line follows). Small spots (menu chip, next to names) keep the text `ww-tier-badge`, since 12px is too small for pixel-art of this detail.
...
- `client-angular/src/app/core/services/avatar.ts` (the `Avatar` service) renders the SVG string and defines the curated per-field option lists (not every DiceBear value is exposed — e.g. eyebrows trimmed to the 7 "Natural" variants, hairstyles curated to 17 — to keep the picker legible rather than overwhelming).
- **Editor**: originally one flat tab per field (11 tabs) in a horizontally-scrolling strip — direct user feedback that the scroll looked broken and the picker felt unmanageable. Regrouped into 4 tabs (`AVATAR_GROUPS` in `categories.ts`: Cara, Pelo, Estilo, Fondo) that fit one row with no scroll; each tab stacks all its fields' sections vertically with a small uppercase label, using ordinary page scroll instead of a nested one. Color fields show plain swatches, shape fields show small rendered avatar thumbnails (the *current* full look with only that one field varied), so picking a hairstyle previews it on the player's actual face/skin/clothes rather than in isolation. Every click re-renders the live big preview immediately.
...
## Menu balance fix: `.friends-section` height

User feedback: the main menu's two columns are `.main-content { grid-template-columns: 1fr 1fr }` — genuinely 50/50 by width already — but visually felt lopsided, "Modos de Juego" cramped on the left against an empty-feeling "Amigos". Root cause was a leftover legacy rule, `.friends-section { height: fit-content }`, which kept the friends panel short while its sibling stretched to the grid row's full height (driven by the taller game-modes content), leaving bare card background in the gap below it. Fixed by letting `.friends-section` fill the row (`height: 100%`) like a normal grid item, and centering `.friends-list`'s content vertically (`flex; justify-content: center`) so the empty state — and a short real friends list — reads as deliberately centered rather than pinned to the top with dead space under it.

## Alignment sweep: auth submit buttons

User-flagged: the login button read as "descuadrado" — narrower than the full-width email/password inputs above it and left-aligned instead of centered or matching their width, because `.btn`/`.btn-primary` never sets a width and a `<button>` defaults to fitting its own text. Fixed with `.auth-form button[type="submit"] { width: 100% }`, which covers all four `.auth-form`s (login, register, forgot-password, reset-password) from one rule. Swept the rest of the app for the same "single narrow button under full-width input(s)" pattern (menu, game-setup, daily challenge, add-friend modal) and found no other instance — the other single/paired buttons in the app sit next to inline inputs or as intentional two-button rows, not stacked under a full-width field.
